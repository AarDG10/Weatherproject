...
from geopy.geocoders import Nominatim  # Nominatim uses OpenStreetMap data to find locations on Earth  (geocoding)
...
def getWeather():
    try:  # try this first
        ...
        location = geolocator.geocode(
            city)  # giving the city name to geolocator.geocode(mumbai)  ...this will store the location of mumbai in a variuable called location
        obj = TimezoneFinder()  # initializing obj variable to TimezoneFinder() module
        result = obj.timezone_at(lng=location.longitude,
                                 lat=location.latitude)  # will give the time zone at the specified latitude and longitude
        ...
        current_time = local_time.strftime(
            "%I:%M %p")  # srtftime is a module of python which returns various time related data...%I=hours as 0 added decimal, %M = minutes as zero added decimal, %p= Will give Am or Pm
        clock.config(
            text=current_time)  # (line no 109)      #putting the value of current_time in  clock  ...config will put the value in the specified label
        name.config(
            text="CURRENT WEATHER")  # (line no 107)   #putting this text in name ...config will put the value in the specified label
        ...
        json_data = requests.get(
            api).json()  # request the data in the api and convert it to json, Store this json is json_data
        ...
        messagebox.showerror("Weather App",
                             "Invalid Input!")  # a message box will pop on the screen if there is any error
...
textfield = tk.Entry(root, justify="center", width=20, font=("poppins", 25, "bold"), bg="black", border=0, fg="yellow",
                     insertbackground="yellow")  # fg=foreground colour(Changes colour of the text) of button,  insertbackground=changes cursor colour
...
myimg_icon = Button(image=search_icon, borderwidth=0, cursor="hand2", bg="yellow",
                    command=getWeather)  # putting a button in the search bar, getWeather function will be called on clicking this
